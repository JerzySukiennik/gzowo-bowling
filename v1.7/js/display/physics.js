  ...
  for (let i = 0; i < GAME.PIN_COUNT; i++) {
    ...
    if ((mask >> i) & 1) {
      ...
      const bellyDesc = RAPIER.ColliderDesc.ball(0.058)
        .setTranslation(0, 0.155, 0).setFriction(0.4).setRestitution(0.05);
      ...
      const neckDesc = RAPIER.ColliderDesc.capsule(0.055, 0.027)
        .setTranslation(0, 0.295, 0).setFriction(0.4).setRestitution(0.05);
...
function substep() {
  ...
  for (let i = 0; i < GAME.PIN_COUNT; i++) {
    ...
    try {
      const av = pin.angvel();
      const am = Math.sqrt(av.x * av.x + av.y * av.y + av.z * av.z);
      if (am > 24) {
        const s = 24 / am;
      ...
      if (lm > 7.5) {
        const ls = 7.5 / lm;
        pin.setLinvel({ x: lv.x * ls, y: lv.y * ls, z: lv.z * ls }, true);
      }
    } catch (e) {}
  }
  if (b) {
