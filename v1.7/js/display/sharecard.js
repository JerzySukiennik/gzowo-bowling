...
import { paintBallDisc } from "../shared/ball3d.js";
...
export function paintBall(ctx, cx, cy, r, color, pattern, patternColor) {
  try {
    const tmp = document.createElement("canvas");
    paintBallDisc(tmp, Math.max(24, Math.round(r * 2)), color, pattern, undefined, patternColor);
    ctx.drawImage(tmp, cx - r, cy - r, r * 2, r * 2);
    return;
  } catch (e) {}
  legacyPaintBall(ctx, cx, cy, r, color, pattern);
}

function legacyPaintBall(ctx, cx, cy, r, color, pattern) {
...
export function ballDiscDataUrl(color, pattern, size, patternColor) {
  try {
    const s = size || 96;
    ...
    paintBall(ctx, s / 2, s / 2, s / 2 - 1, color, pattern, patternColor);
...
export async function generateCard(result, lang) {
  ...
  paintBall(ctx, 270, 490, 60, topRow.ballColor, topRow.ballPattern, topRow.ballPatternColor);
  ctx.textAlign = "left";
  ctx.fillStyle = INK2;
  ...
  for (let i = 0; i < ranking.length; i++) {
    ...
    paintBall(ctx, 235, y + 42, 28, row.ballColor, row.ballPattern, row.ballPatternColor);
    if (isFirst) {
      ctx.font = "32px serif";
