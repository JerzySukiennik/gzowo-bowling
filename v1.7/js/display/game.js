...
let chooseTimer = 0;
let chooseBaseTs = 0;
let skipChooseOnce = false;

function ballsOf(pid) {
  const p = players[pid] || {};
  let arr = Array.isArray(p.balls) ? p.balls.filter((b) => b && typeof b === "object") : [];
  if (!arr.length) {
    arr = [{
      color: p.ballColor || "#FF6B6B",
      pattern: p.ballPattern || "solid",
      patternColor: p.ballPatternColor || undefined,
      weight: (typeof p.ballWeight === "number" ? p.ballWeight : 10),
    }];
  }
  return arr.slice(0, 3);
}

function ballAt(pid, idx) {
  const arr = ballsOf(pid);
...
function writeGame() {
  net.update("game", {
    ...
    chooseFor: chooseFor || null,
    chooseCount: chooseFor ? ballsOf(chooseFor).length : 0,
  });
}
  ...
  if (gameStarted) {
    ...
    for (const pid of orderIds) {
      ...
      if (prevConnected[pid] === true && p.connected === false) {
        ...
        if (pid === cur.playerId && (phase === "aiming" || phase === "nextTurn" || phase === "chooseBall")) skipPid = pid;
...
function maybeEnterChooseBall() {
  const pid = cur.playerId;
  const p = players[pid];
  if (!p || p.connected === false) { autoSkip(pid); return; }
  if (skipChooseOnce) { skipChooseOnce = false; enterAiming(); return; }
  const isFirstThrow = currentThrowList().length === 0;
  if (!isFirstThrow || ballsOf(pid).length <= 1) { enterAiming(); return; }
  enterChooseBall(pid);
}

function enterChooseBall(pid) {
  chooseFor = pid;
  const inp = inputAll[pid] || {};
  chooseBaseTs = inp.ballChoice && inp.ballChoice.ts ? inp.ballChoice.ts : 0;
  chooseTimer = TIMING.CHOOSE_BALL_MS;
  try { D.camera.flyToRack(laneOf(pid)); } catch (e) {}
  try { D.scene.showBallRack(laneOf(pid), ballsOf(pid), chosenBallIdx[pid] | 0); } catch (e) {}
  setPhase("chooseBall");
  writeGame();
  refreshHud();
}

function onBallChoiceReceived(pid, choice) {
  if (phase !== "chooseBall" || pid !== chooseFor || pausedFlag) return;
  if (!choice || typeof choice.ts !== "number" || choice.ts <= chooseBaseTs) return;
  chooseBaseTs = choice.ts;
  const n = ballsOf(pid).length;
  const idx = Math.max(0, Math.min(n - 1, Number(choice.index) | 0));
  chosenBallIdx[pid] = idx;
  try { D.scene.highlightRackBall(idx); } catch (e) {}
  D.audio.sfx("ding", 0.6);
  commitChosenBallVisual(pid);
  finishChooseBall(pid);
}

function finishChooseBall(pid) {
  chooseFor = null;
  chooseTimer = 0;
  try { D.scene.hideBallRack(); } catch (e) {}
  enterAiming();
}

function commitChosenBallVisual(pid) {
  try {
    const b = ballAt(pid, chosenBallIdx[pid] | 0);
    const mesh = D.scene.createBallMesh(b.color, b.pattern, b.weight, b.patternColor);
    D.scene.setBallVisual(laneOf(pid), mesh);
  } catch (e) {}
}
...
function releaseThrow(inp) {
  const pid = cur.playerId;
  const p = players[pid] || {};
  const chosenBall = ballAt(pid, chosenBallIdx[pid] | 0);
  const th = inp.throw;
  const spin = spinFor(pid);
  const aim = lastAimFor(pid);
  throwCtx = { pid, spin, weightKey: (typeof chosenBall.weight === "number" ? chosenBall.weight : (p.ballWeight || "medium")) };
  preMask = rackMask;
  undoState = snapshot;
...
function finishGame(winnerIds, sdWinner) {
  ...
  const ranking = ranked.map((pid) => {
    ...
    return {
      ...
      ballPatternColor: p.ballPatternColor,
    };
  });
  const result = {
...
export function onInput(map) {
  ...
  } else if (phase === "chooseBall" && chooseFor) {
    const inp = inputAll[chooseFor];
    if (inp && inp.ballChoice) onBallChoiceReceived(chooseFor, inp.ballChoice);
  }
}
...
export function tick(dtMs) {
  ...
  if (phase === "intro") {
    ...
    turnTimer -= dtMs;
    if (turnTimer <= 0) maybeEnterChooseBall();
  } else if (phase === "chooseBall") {
    const cp = players[chooseFor];
    if (!cp || cp.connected === false) {
      const pid = chooseFor;
      chosenBallIdx[pid] = 0;
      commitChosenBallVisual(pid);
      finishChooseBall(pid);
