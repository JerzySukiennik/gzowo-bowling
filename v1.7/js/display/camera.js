...
export function setMode() {
  // Camera is permanently in "auto" mode; manual view switching removed. No-op kept for call-site compatibility.
}
...
export function flyToRack(laneIndex) {
  return new Promise((resolve) => {
    ...
    fly = {
      from: camera.position.clone(),
      fromLook: look.clone(),
      to: new THREE.Vector3(laneX - 3.4, 1.05, 4.4),
      toLook: new THREE.Vector3(laneX - 1.9, 0.55, 3.0),
      t: 0,
      dur: TIMING.NEXT_TURN_FLY_MS,
      resolve,
    };
...
export function update(dtMs, ctx) {
  ...
  if (phase === "intro" || phase === "lobby") {
    ...
    posTarget.set(laneX - 3.4, 1.05, 4.4);
    lookTarget.set(laneX - 1.9, 0.55, 3.0);
