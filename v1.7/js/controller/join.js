...
import { BALL_COLORS, BALL_PATTERNS, LANE_THEMES, THEME_CONFIG, BALL_WEIGHT_RANGE } from "../config.js";
import { t } from "../i18n.js";
import { el, clear, sfx, vib, popNode, shakeNode, paintBallDisc } from "./ui.js";
import { initBallPreview } from "./ballpreview.js";

function weightHintKey(w) {
  return w <= 8 ? "join.weightHint.light" : w >= 13 ? "join.weightHint.heavy" : "join.weightHint.medium";
}
...
export function showJoin(rootEl, onComplete) {
  const state = {
    ...
    ballPattern: "solid",
    ballPatternColor: null,
    ballWeight: BALL_WEIGHT_RANGE.DEFAULT,
    laneTheme: "classic",
  };
  let step = 1;
  let preview = null;
  ...
  function renderStep2() {
    ...
    function repaintChips() {
      for (const [p, mini] of chipCanvases) paintBallDisc(mini, 72, state.ballColor, p, undefined, state.ballPatternColor);
    }
    repaintChips();

    const patternColorLabel = el("div", "label label-upper", t("join.patternColor"));
    patternColorLabel.style.margin = "16px 0 8px";
    const patternColorRow = el("div", "swatch-row");
    const pcSwatches = new Map();
    function selectPatternColor(c) {
      state.ballPatternColor = c;
      for (const [cc, node] of pcSwatches) node.classList.toggle("selected", c === null ? cc === "auto" : cc === c);
      repaintChips();
      applyBall();
    }
    const autoSw = el("button", "swatch" + (state.ballPatternColor === null ? " selected" : ""));
    autoSw.style.background = "conic-gradient(#FF5C7A,#FFD23F,#3DD68C,#4CC9F0,#9B5DE5,#FF5C7A)";
    autoSw.setAttribute("aria-label", t("join.patternAuto"));
    autoSw.addEventListener("click", () => { sfx("ui"); vib(10); popNode(autoSw); selectPatternColor(null); });
    pcSwatches.set("auto", autoSw);
    patternColorRow.append(autoSw);
    for (const c of BALL_COLORS) {
      const s = el("button", "swatch" + (c === state.ballPatternColor ? " selected" : ""));
      s.style.background = c;
      s.addEventListener("click", () => { sfx("ui"); vib(10); popNode(s); selectPatternColor(c); });
      pcSwatches.set(c, s);
    ...
    const valueNum = el("div", "weight-value", String(state.ballWeight));
    ...
    slider.step = "1";
    slider.value = String(state.ballWeight);
    function setWeight(w, bump) {
      w = Math.min(BALL_WEIGHT_RANGE.MAX, Math.max(BALL_WEIGHT_RANGE.MIN, Math.round(w)));
      if (w === state.ballWeight && !bump) return;
      state.ballWeight = w;
      valueNum.textContent = String(w);
      slider.value = String(w);
      weightHint.textContent = t(weightHintKey(w));
      popNode(valueNum);
      applyBall();
      sfx("ui");
      vib(10);
    }
    minus.addEventListener("click", () => setWeight(state.ballWeight - 1, true));
    plus.addEventListener("click", () => setWeight(state.ballWeight + 1, true));
    slider.addEventListener("input", () => setWeight(Number(slider.value)));
    weightRow.append(minus, valueWrap, plus);
    const weightHint = el("div", "weight-hint", t(weightHintKey(state.ballWeight)));
    ...
    dock.append(next);
    wrap.append(colorLabel, swatchRow, patternLabel, patternGrid, patternColorLabel, patternColorRow, weightLabel, weightRow, slider, weightHint, dock);
    shell(wrap);
    ...
    function applyBall() {
      if (preview) preview.setBall(state.ballColor, state.ballPattern, state.ballWeight, state.ballPatternColor);
    }
  }
  ...
  function renderStep3() {
    ...
    ready.addEventListener("click", () => {
      ...
      onComplete({
        ...
        ballPatternColor: state.ballPatternColor,
