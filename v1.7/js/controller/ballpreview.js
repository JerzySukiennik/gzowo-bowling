...
export function initBallPreview(canvasEl) {
  ...
  let bounceAmp = 0;
  let fallbackAngle = 0;
  const state = { color: "#FF6B6B", pattern: "solid", weight: BALL_WEIGHT_RANGE.DEFAULT, patternColor: null };
  ...
  function fallbackDraw() {
    try {
      ...
      paintBallDisc(tmp, Math.round(size), state.color, state.pattern, state.weight, state.patternColor);
  ...
  function applyAppearance() {
    if (mode === "3d" && ballApi) ballApi.setAppearance(state.color, state.pattern, state.weight, state.patternColor);
  }
  ...
  async function boot() {
    try {
      ...
      ballApi = buildBall3D(THREE, { radius: 1, color: state.color, pattern: state.pattern, weight: state.weight, patternColor: state.patternColor, gradientMap: gradient });
  ...
  return {
    setBall(color, pattern, weight, patternColor) {
      const w = Number(weight);
      const weightChanged = Number.isFinite(w) && w !== state.weight;
      if (color) state.color = color;
      if (pattern) state.pattern = pattern;
      if (patternColor !== undefined) state.patternColor = patternColor;
