...
export function initHud(containerEl) {
  ...
  hudEl.append(leaderboardEl, plaqueLayer, bannerSlot, letterTop, letterBottom);
...
function renderConnect(data) {
  const screen = el("div", "connect-screen connect-hostfirst");
  const logo = el("div", "logo-hero");
  logo.append(el("span", "pin-glyph", "🎳"), el("span", null, t("app.title").toUpperCase()));
  const title = el("div", "hostfirst-title spring-in", t("connect.hostFirstTitle"));
  const sub = el("div", "hostfirst-sub", t("connect.hostFirstSub"));
  const qrBox = makeQr(data.url, 420);
  qrBox.classList.add("qr-box-lg", "spring-in");
  const caption = el("div", "qr-caption hostfirst-caption", t("connect.hostFirstQrCaption"));
  const codeLine = el("div", "hostfirst-code qr-code-mini", t("lobby.roomCode") + ": " + (data.code || ""));
  screen.append(logo, title, sub, qrBox, codeLine, caption);
  return screen;
}
...
export function popEmote(laneIndex, emoteChar) {
  ...
  st.active++;
  const base = lanePos[laneIndex];
  let px, py;
  if (base && base.y > window.innerHeight * 0.2 && base.y < window.innerHeight - 30) {
    px = base.x;
    py = base.y;
  } else {
    px = 90 + laneIndex * 76;
    py = window.innerHeight * 0.6;
  }
  px = Math.max(80, Math.min(window.innerWidth - 80, px + (Math.random() - 0.5) * 44));
  py = Math.max(window.innerHeight * 0.34, Math.min(window.innerHeight - 100, py - 20));
  const outer = el("div", "emote-pop");
  outer.style.cssText =
    "position:fixed;z-index:40;pointer-events:none;font-size:96px;line-height:1;" +
