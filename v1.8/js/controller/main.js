...
let installHintBar = null;

function maybeShowInstallHint() {
  try {
    const standalone = (window.matchMedia && (window.matchMedia("(display-mode: fullscreen)").matches || window.matchMedia("(display-mode: standalone)").matches)) || window.navigator.standalone === true;
    if (standalone) return;
    if (sessionStorage.getItem("gzowo.pwahint") === "1") return;
    if (installHintBar) return;
    const bar = el("div", "pwa-hint spring-in");
    const txt = el("span", null, t("pwa.installHint"));
    const close = el("button", null, t("pwa.installClose"));
    close.addEventListener("click", () => {
      try { sessionStorage.setItem("gzowo.pwahint", "1"); } catch (e) {}
...
function hideInstallHint() {
  if (installHintBar) { try { installHintBar.remove(); } catch (e) {} installHintBar = null; }
}
...
function renderLobby() {
  ...
  if (my) {
    ...
    info.append(meta2);
    youCard.append(info);
    const mbBtn = el("button", "btn btn-3d btn-pill btn-myballs", t("myballs.button"));
    mbBtn.addEventListener("click", () => { sfx("ui"); vib(10); popNode(mbBtn); openMyBalls(); });
    youCard.append(mbBtn);
...
function ensureGameView() {
  ...
  statusBar.append(strip, title, frameChip, pausedChip, tut, burgerSlot);
  ...
  const mbGameBtn = el("button", "btn btn-3d btn-pill btn-myballs btn-myballs-game", t("myballs.button"));
  mbGameBtn.addEventListener("click", () => { sfx("ui"); vib(10); popNode(mbGameBtn); openMyBalls(); });
  waitRoot.append(turnChip, scoreCard, ticker, mbGameBtn);
  ...
  gameView = { root, title, frameChip, pausedChip, throwRoot, waitRoot, pad, turnChip, turnDisc, turnNick, bigTotal, frameStrip, rankChip, ticker, emoteWrap, strip };
  prevTurnKey = "";
  prevTurnPlayer = null;
  prevRank = 0;
}
