// Gzowo Bowling — interactive 7-step pad tutorial overlay: my-balls intro + try-it aim/spin/flick steps, hand hints, skippable.
import { EMOTES } from "../config.js";
import { t, onLangChange } from "../i18n.js";
import { el, clear, sfx, vib, popNode, paintBallDisc } from "./ui.js";
...
const STEPS = ["myballs", "aim", "spin", "flick", "emotes", "skipTurn", "mulligan"];
...
export function startTutorial(rootEl, onDone) {
  ...
  function mockMyBallsStep() {
    const wrap = el("div", "tut-balls card spotlit");
    const demo = [
      { color: "#EF476F", pattern: "stripes", weight: 10, patternColor: null },
      { color: "#4CC9F0", pattern: "dots", weight: 12, patternColor: "#FFD166" },
      { color: "#8338EC", pattern: "swirl", weight: 14, patternColor: null },
    ];
    demo.forEach((ball, i) => {
      const slot = el("div", "tut-ball" + (i === 1 ? " sel" : ""));
      const c = el("canvas", "ball-disc-canvas");
      c.style.width = "80px";
      c.style.height = "80px";
      paintBallDisc(c, 160, ball.color, ball.pattern, ball.weight, ball.patternColor);
      slot.append(c);
      if (i === 1) slot.append(el("span", "tut-ball-check", "✓"));
      wrap.append(slot);
    });
    stage.append(wrap);
    placeHand("");
