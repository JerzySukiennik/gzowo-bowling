...
export function makeBallTexture(color, pattern, size, patternColor) {
  const w = size || 1024;
...
export function buildBall3D(THREE, opts) {
  ...
  const mat = new THREE.MeshStandardMaterial({ vertexColors: true, roughness: 0.22, metalness: 0.0, envMapIntensity: 1.15 });
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    for (let h = 0; h < 3; h++) {
      ...
      if (ang < ha) {
        const rim = smooth(0.62, 1, ang / ha);
        scale = Math.min(scale, 1 - 0.22 * (1 - rim));
  ...
  let texture = null;
  let numTexture = null;
  function setAppearance(color, pattern, weight, patternColor) {
    try {
      const canvas = makeBallTexture(color, pattern, undefined, patternColor);
  ...
  setAppearance(opts.color || "#FF6B6B", opts.pattern || "solid", opts.weight, opts.patternColor);
