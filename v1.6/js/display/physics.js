  ...
  for (let i = 0; i < GAME.PIN_COUNT; i++) {
    ...
    if ((mask >> i) & 1) {
      ...
      const baseDesc = RAPIER.ColliderDesc.cylinder(0.07, 0.052)
        .setTranslation(0, 0.07, 0).setFriction(0.45).setRestitution(0.18);
      ...
      const bellyDesc = RAPIER.ColliderDesc.ball(0.058)
        .setTranslation(0, 0.155, 0).setFriction(0.4).setRestitution(0.22);
      ...
      const neckDesc = RAPIER.ColliderDesc.capsule(0.055, 0.027)
        .setTranslation(0, 0.295, 0).setFriction(0.4).setRestitution(0.22);
...
function substep() {
  ...
  world.step();
  for (let i = 0; i < GAME.PIN_COUNT; i++) {
    const pin = active.pins[i];
    if (!pin) continue;
    try {
      const av = pin.angvel();
      const m = Math.sqrt(av.x * av.x + av.y * av.y + av.z * av.z);
      if (m > 40) {
        const s = 40 / m;
        pin.setAngvel({ x: av.x * s, y: av.y * s, z: av.z * s }, true);
        const lv = pin.linvel();
        const lm = Math.sqrt(lv.x * lv.x + lv.y * lv.y + lv.z * lv.z);
        if (lm > 14) { const ls = 14 / lm; pin.setLinvel({ x: lv.x * ls, y: lv.y * ls, z: lv.z * ls }, true); }
      }
    } catch (e) {}
  }
  if (b) {
