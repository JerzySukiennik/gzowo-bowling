...
function buildHall(THREE, makeToonMat, laneCount) {
  ...
  signBoard.position.set(centerX, 3.4, FRONT_Z - 0.22);
  add(signBoard);
  ...
  hall.add(sign);
  const poster = (x, y, z) => {
    const p = box(0.06, 1.1, 0.8, makeToonMat(COL.poster));
    p.position.set(x, y, z); add(p);
    const frame = box(0.04, 1.2, 0.9, makeToonMat("#E9C98A"));
    frame.position.set(x + (x < centerX ? 0.01 : -0.01), y, z); add(frame);
  };
...
export function showBallRack(laneIndex, balls, chosenIdx) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const g = api.group;
    g.position.set(laneX + (i - (n - 1) / 2) * spacing, shelfY + LANE.BALL_RADIUS, shelfZ);
