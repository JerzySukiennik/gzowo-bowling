...
let pauseReplayTimer = 0;
let pauseReplayAlt = true;
...
export function tickPaused(dtMs) {
  if (!D || !pausedFlag || !gameStarted) return;
  if (!D.replay.getBestMeta()) return;
  pauseReplayTimer -= dtMs;
  if (pauseReplayTimer <= 0 && !D.replay.isReplaying()) {
    pauseReplayTimer = 999999;
    const player = pauseReplayAlt && D.replay.getBestMeta() ? D.replay.playBest() : D.replay.playLast();
    pauseReplayAlt = !pauseReplayAlt;
    player.then(() => {
      pauseReplayTimer = 1200;
    }).catch(() => {
      pauseReplayTimer = 4000;
    });
  }
}
...
function enterAiming() {
  ...
  D.scene.showAimGhost(lane, lastAimFor(pid), spinFor(pid));
  snapshot = {
    playerId: pid,
...
function resolveThrow() {
  ...
  const isStrike = wasFullRack && pinsDown === GAME.PIN_COUNT;
  const isSpare = !isStrike && pinsStanding === 0 && pinsDown > 0 && !wasFullRack;
