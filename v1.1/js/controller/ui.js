...
export function paintBallDisc(canvas, size, color, pattern, weight) {
  try {
    paintDisc(canvas, size, color, pattern, weight);
  } catch (e) {}
}

const discCache = new Map();

export function ballDiscUrl(color, pattern, size, weight) {
  const s = size || 96;
  const key = color + "|" + pattern + "|" + s + "|" + (weight || "");
  if (discCache.has(key)) return discCache.get(key);
  let url = "";
  try {
    const c = document.createElement("canvas");
    paintDisc(c, s * 2, color, pattern, weight);
