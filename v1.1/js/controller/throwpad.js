...
export function initThrowPad(rootEl) {
  ...
  rootEl.classList.add("throw-grid");
  ...
  return {
    ...
    setPins(standingCount) {
      const down = Math.max(0, Math.min(10, 10 - (typeof standingCount === "number" ? standingCount : 10)));
      pinDots.forEach((d, i) => d.classList.toggle("down", i < down));
    },
