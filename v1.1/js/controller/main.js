...
function attachWatchers() {
  ...
  net.watch("meta", (v) => {
    ...
    if (meta.settings && meta.settings.language) setLang(meta.settings.language);
...
function renderLobby() {
  ...
  if (my) {
    ...
    youCard.append(ballDiscEl(my.ballColor, my.ballPattern, "ball-disc-72"));
    const info = el("div");
  ...
  if (others.length) {
    ...
    for (const p of others) {
      ...
      row.append(ballDiscEl(p.ballColor, p.ballPattern, "ball-disc-32"));
  ...
  if (isHost) {
    ...
    card.append(segmented(
      ...
      (v) => {
        net.write("meta/settings/language", v);
        setLang(v);
      }
    ));
...
function ensureGameView() {
  ...
  const root = el("div", "throwpad game-landscape");
  root.addEventListener("pointerdown", tryLockLandscape, { once: true });
  const rotateGate = el("div", "rotate-gate");
  rotateGate.append(el("div", "rotate-icon", "📱"), el("div", null, t("pad.rotate")));
  root.append(rotateGate);
  ...
  statusBar.append(strip, title, frameChip, pausedChip, cameraBtn);
  ...
  bottomBar.append(emoteWrap, tut, burgerSlot);
  ...
  if (my) {
    ...
    pad.setBall(my.ballColor, my.ballPattern);
...
function updateGame() {
  ...
  if (my) {
    ...
    gameView.pad.setBall(my.ballColor, my.ballPattern);
  ...
  if (turnKey !== prevTurnKey) {
    ...
    if (myTurn) {
      ...
      if ((g.throwIndex || 0) > 0 && lr && lr.playerId === myId && lr.frameIndex === g.currentFrameIndex) {
        gameView.pad.setPins(lr.pinsStanding);
      } else {
        gameView.pad.setPins(10);
      }
    }
  ...
  if (cur) gameView.turnDisc.append(ballDiscEl(cur.ballColor, cur.ballPattern, "ball-disc-48"));
...
function renderWinner() {
  ...
  nameRow.append(ballDiscEl(winner.ballColor || "#FF6B6B", winner.ballPattern || "solid", "ball-disc-48"));
  ...
  ranking.forEach((row, i) => {
    ...
    r.append(ballDiscEl(row.ballColor || "#FF6B6B", row.ballPattern || "solid", "ball-disc-32"));
