...
export function initThrowPad(rootEl) {
  ...
  function layout() {
    ...
    const startY = y - 24;
    const offset = state.spin * g.halfW * 0.9;
    const endX = Math.max(g.centerX - g.halfW, Math.min(g.centerX + g.halfW, x + offset));
    const cpX = x;
    const cpY = (startY + endY) / 2;
    arrowPath.setAttribute("d", "M " + x + " " + startY + " Q " + cpX + " " + cpY + " " + endX + " " + endY);
    layoutPins();
  }
  ...
  function updateSpinUi(snap) {
    ...
    const place = () => {
      ...
      if (w < 64) { requestAnimationFrame(place); return; }
      knob.style.left = ((state.spin + 1) / 2 * (w - 64)) + "px";
    };
  ...
  return {
    ...
    setBall(color, pattern, patternColor) {
      const url = ballDiscUrl(color, pattern, 112, patternColor);
      if (url) {
