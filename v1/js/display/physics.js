...
import { LANE, PHYSICS, BALL_WEIGHTS, GAME } from "../config.js";
...
const active = {
  ...
  pinVis: null,
  ball: null,
  ballWeight: BALL_WEIGHTS.medium,
  spin: 0,
  hasThrow: false,
  touchedBumper: false,
  inGutter: false,
  ...
  for (let i = 0; i < GAME.PIN_COUNT; i++) {
    ...
    if ((mask >> i) & 1) {
      ...
      const bodyDesc = RAPIER.RigidBodyDesc.dynamic()
        .setTranslation(laneX + spot.x, PIN_HH, spot.z)
        .setLinearDamping(0.12)
        .setAngularDamping(0.25);
      const body = world.createRigidBody(bodyDesc);
      const colDesc = RAPIER.ColliderDesc.cylinder(PIN_HH, LANE.PIN.RADIUS).setFriction(0.4).setRestitution(0.4);
      setColliderMass(colDesc, LANE.PIN.MASS, Math.PI * LANE.PIN.RADIUS * LANE.PIN.RADIUS * LANE.PIN.HEIGHT);
...
export function throwBall(laneIndex, throwData, weightKey) {
  if (!world) return;
  const td = throwData || {};
  const W = BALL_WEIGHTS[weightKey] || BALL_WEIGHTS.medium;
  const aim = td.aim || {};
...
export function getPinTransforms(laneIndex) {
  ...
  for (let i = 0; i < GAME.PIN_COUNT; i++) {
    ...
    try {
      ...
      const yx = 2 * (q.x * q.y - q.z * q.w);
      const yy = 1 - 2 * (q.x * q.x + q.z * q.z);
      const yz = 2 * (q.y * q.z + q.x * q.w);
      out.push({
        p: [p.x - yx * PIN_HH, p.y - yy * PIN_HH, p.z - yz * PIN_HH],
        q: [q.x, q.y, q.z, q.w],
      });
    } catch (e) {
      out.push({ p: [laneX + PIN_SPOTS[i].x, -5, PIN_SPOTS[i].z], q: [0, 0, 0, 1] });
