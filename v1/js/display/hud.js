...
function renderLobby(data) {
  ...
  entries.forEach(([pid, p], i) => {
    ...
    if (p.ready) {
      ...
      const wKey = p.ballWeight === "light" || p.ballWeight === "heavy" ? p.ballWeight : "medium";
      metaRow.append(el("span", "weight-badge", t("join.weight." + wKey)));
...
export function updatePlaques(playersArr, scoresMap, activePlayerId) {
  ...
  (playersArr || []).forEach((p) => {
    ...
    if (!pl) {
      const wrap = el("div", "plaque spring-in");
...
export function setPauseOverlay(on) {
  if (on && !pauseEl) {
    pauseEl = el("div", "pause-overlay");
    const card = el("div", "card pause-card spring-in");
    card.append(
      el("div", "pause-icon", "⏸"),
      el("div", "pause-title", t("hud.paused")),
      el("p", "muted", t("pad.paused"))
    );
    pauseEl.append(card);
...
export function toast(msg) {
  try {
    if (!toastEl) {
      ...
      toastEl.style.cssText = "position:fixed;left:50%;bottom:24px;transform:translateX(-50%);z-index:85;font-size:var(--fs-body);box-shadow:var(--shadow);";
